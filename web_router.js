...
router.get('/user/:name/collections', user.listCollectedTopics);  // 用户收藏的所有情书页
router.get('/user/:name/topics', user.listTopics);  // 用户发布的所有情书页
...
router.get('/topic/:tid', topic.index);  // 显示某个情书
router.post('/topic/:tid/top', auth.adminRequired, topic.top);  // 将某情书置顶
router.post('/topic/:tid/good', auth.adminRequired, topic.good); // 将某情书加精
router.get('/topic/:tid/edit', auth.userRequired, topic.showEdit);  // 编辑某情书
...
router.post('/topic/collect', auth.userRequired, topic.collect); // 关注某情书
router.post('/topic/de_collect', auth.userRequired, topic.de_collect); // 取消关注某情书
